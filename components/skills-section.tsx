          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {skillCategories.map((category) => {
              ...
              return (
                <Card
                  key={category.title}
                  className={`glass-card glass-card-hover rounded-3xl border border-border overflow-hidden bg-gradient-to-br ${category.gradient}`}
                >
                  ...
                  <CardContent>
                    <div className="flex flex-wrap gap-2">
                      {category.skills.map((skill) => (
                        <span
                          key={skill}
                          className="px-3.5 py-1.5 rounded-xl text-xs font-semibold bg-secondary/80 text-foreground border border-border/80 hover:border-primary/50 hover:bg-primary/10 transition-all duration-300 shadow-sm cursor-default"
                        >
                          {skill}
                        </span>
                      ))}
